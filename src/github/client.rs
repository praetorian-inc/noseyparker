...
use super::{Auth, ClientBuilder, Error, Result, models};
...
impl Client {
    ...
    pub async fn get_rate_limit(&self) -> Result<models::RateLimitOverview> {
        let response = self.get(&["rate_limit"]).await?;
        let body = response.json().await.map_err(Error::ReqwestError)?;
        Ok(body)
    }

    pub async fn get_user(&self, username: &str) -> Result<models::User> {
        let response = self.get(&["users", username]).await?;
        let body = response.json().await.map_err(Error::ReqwestError)?;
        Ok(body)
    }

    pub async fn get_user_repos(&self, username: &str) -> Result<models::Page<models::Repository>> {
        let response = self.get_with_params(&["users", username, "repos"], &[("per_page", "100")]).await?;
        let body = models::Page::from_response(response).await?;
        Ok(body)
    }

    pub async fn next_page<T>(&self, page: models::Page<T>) -> Result<Option<models::Page<T>>>
        where T: serde::de::DeserializeOwned
    {
        if let Some(next) = page.links.next {
            let response = self.get_url(next).await?;
            let page = models::Page::from_response(response).await?;
            Ok(Some(page))
        } else {
            Ok(None)
        }
...
impl Client {
    ...
    fn make_url(&self, path_parts: &[&str], params: &[(&str, &str)]) -> Result<Url> {
        ...
        let url = self.base_url.clone().join(&buf).map_err(Error::UrlParseError)?;
    ...
    async fn get_with_params(&self, path_parts: &[&str], params: &[(&str, &str)]) -> Result<reqwest::Response> {
        ...
        if response.status() == StatusCode::FORBIDDEN {
            ...
            if let Some(b"0") = response.headers().get("x-ratelimit-remaining").map(HeaderValue::as_bytes) {
                let wait = || -> Option<Duration> {
                    ...
                    let reset_time = response.headers().get("x-ratelimit-reset")?.to_str().ok()?.parse::<i64>().ok()?;
